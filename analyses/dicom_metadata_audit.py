...
def write_report(
    ...
    (HERE / "dicom_metadata_audit_report.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Read-only DICOM acquisition metadata audit."
    )
    parser.add_argument(
...
def main() -> None:
    ...
    write_csv(HERE / "series_metadata.csv", series_rows, SERIES_FIELDS)
    summary_fields = list(summary_rows[0])
    write_csv(
        HERE / "institution_summary.csv", summary_rows, summary_fields
    )
