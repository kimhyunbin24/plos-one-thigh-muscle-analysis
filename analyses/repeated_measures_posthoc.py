...
def load_analysis_rows() -> list[dict[str, float | str]]:
    default_path = PROJECT_ROOT / "data" / "analysis_data.sav"
    return read_spss_sav(Path(os.environ.get("CT_VOLUME_SAV_PATH", default_path)))


def values(rows: list[dict[str, object]], name: str) -> np.ndarray:
    return np.asarray([float(row.get(name, math.nan)) for row in rows], dtype=float)
...
def student_t_critical_975(degrees_freedom: int) -> float:
    ...
    for _ in range(100):
        mid = (low + high) / 2.0
        cdf = 1.0 - student_t_two_sided_p(mid, degrees_freedom) / 2.0
        if cdf < 0.975:
            low = mid
        else:
            high = mid
...
def main() -> None:
    ...
    output_dir = OUTPUT_DIR / "repeated_measures"
    output_dir.mkdir(parents=True, exist_ok=True)
    write_csv(output_dir / "compartment_posthoc.csv", compartment_results)
    write_csv(output_dir / "individual_muscle_posthoc.csv", muscle_results)
    write_word(output_dir / "repeated_measures_results.docx", omnibus, compartment_results, muscle_results)
    ...
    (output_dir / "analysis_summary.txt").write_text(response, encoding="utf-8")
    ...
    print(output_dir / "repeated_measures_results.docx")
